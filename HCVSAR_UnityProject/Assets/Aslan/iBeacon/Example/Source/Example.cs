...
internal class Example : MonoBehaviour {
	...
	float GetDistance(Beacon b) {
		int iRssi = Math.Abs(b.rssi);
		float power = (float)(iRssi - 54) / (10 * 2);
		return Mathf.Pow(10, power);
	}

	Vector2 GetCoordinate(double d1, double d2)
	{
		//float ux;
		//float uy;

		double x = (float)Math.Pow(d1, 2) - Math.Pow(d2, 2) + Math.Pow(PointB.x, 2);
		//ux = (float)x*10 / 2 * PointB.x;
		//uy = (float)Math.Sqrt(Math.Pow(d1*100, 2) - Math.Pow(ux, 2));
		userPoint.x = (float)x * 10 / 2 * PointB.x;
		userPoint.y = (float)Math.Sqrt(Math.Pow(d1 * 100, 2) - Math.Pow(userPoint.x, 2));
	...
	private void DisplayOnBeaconFound() {
		...
		foreach (Beacon b in mybeacons) {
			...
			case BeaconType.iBeacon:
				...
				Texts[6].text = "Coordinate"; //"Range:";
				Texts[7].text = GetCoordinate(d1,d2).ToString();//b.range.ToString();
		...
		txt_BroadcastState_LabelText.text = "( " + GetCoordinate(d1, d2).x.ToString() + " , " + GetCoordinate(d1, d2).y.ToString() + " )";
	}
